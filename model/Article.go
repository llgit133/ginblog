...
// GetArt 查询文章列表
// select * from article left join category on article.cid = category.id  limit ? offset ? order by created_at desc
func GetArt(pageSize int, pageNum int) ([]Article, int, int64) {
	var articleList []Article
	var err error
	var total int64

	//err = db.Select("article.cid, title, img, created_at, updated_at, `desc`, comment_count, read_count, category.name").Limit(pageSize).Offset((pageNum - 1) * pageSize).Order("Created_At DESC").Joins("Category").Find(&articleList).Error
	err = db.
		Table("article").
		Select("article.cid, article.title, article.img, article.created_at, article.updated_at, article.desc, article.comment_count, article.read_count, category.name").
...
// select * from article left join  where title like ? limit ? offset ?
...
// EditArt 编辑文章
// update article set title = ?,cid = ?,desc = ?,content = ?,img = ? where id = ?
